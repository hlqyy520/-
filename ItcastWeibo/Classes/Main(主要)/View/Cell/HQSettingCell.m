 ...
 *  提醒数字
 */
@property (strong, nonatomic) HQBadgeButton *badgeButton;
@end

@implementation HQSettingCell

- (HQBadgeButton *)badgeButton
{
    if (_badgeButton == nil) {
        _badgeButton = [[HQBadgeButton alloc] init];
    }
    return _badgeButton;
}
...
{
    // 存储数据
    HQSettingSwitchItem *switchItem = (HQSettingSwitchItem *)self.item;
...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    if (cell == nil) {
        cell = [[HQSettingCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
...
{
    if (iOS7) {
        frame.origin.x = HQSettingTableBorder;
        frame.size.width -= 2 * HQSettingTableBorder;
    } else {
        CGFloat x = HQSettingTableBorder - 9;
        frame.origin.x = x;
...
- (void)setupRightView
{
    ...
    else if ([self.item isKindOfClass:[HQSettingLabelItem class]])
    { // 右边是文字
        self.accessoryView = nil;
        HQSettingLabelItem *labelItem = (HQSettingLabelItem *)self.item;
        self.detailTextLabel.text = labelItem.text;
    }
    else if ([self.item isKindOfClass:[HQSettingSwitchItem class]])
    { // 右边是开关
        HQSettingSwitchItem *switchItem = (HQSettingSwitchItem *)self.item;
        self.switchView.on = switchItem.isOn;
        self.accessoryView = self.switchView;
    }
    else if ([self.item isKindOfClass:[HQSettingArrowItem class]])
    { // 右边是箭头
        self.accessoryView = self.arrowView;
    }
    else if ([self.item isKindOfClass:[HQSettingCheckItem class]])
    { // 右边是打钩
        HQSettingCheckItem *checkItem = (HQSettingCheckItem *)self.item;
