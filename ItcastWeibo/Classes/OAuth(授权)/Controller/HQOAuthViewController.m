...
#import "HQOAuthViewController.h"
...
@interface HQOAuthViewController () <UIWebViewDelegate>

@end

@implementation HQOAuthViewController
...
- (void)viewDidLoad
{
    ...
    // 2.加载授权页面(新浪提供的登录页面)
    NSURL *url = [NSURL URLWithString:HQLoginURL];
...
{
    // 1.请求的URL路径: http://www.baidu.com/?code=0f189b682cd020e79303dbb043d4fb28
...
- (void)accessTokenWithCode:(NSString *)code
{
    ...
    params[@"client_id"] = HQAppKey;
    params[@"client_secret"] = HQAppSecret;
    params[@"grant_type"] = @"authorization_code";
    params[@"code"] = code;
    params[@"redirect_uri"] = HQRedirectURI;
    
    // 3.发送请求
    [mgr POST:@"https://api.weibo.com/oauth2/access_token" parameters:params
      success:^(AFHTTPRequestOperation *operation, id responseObject) {
          // 4.先将字典转为模型
          HQAccount *account = [HQAccount accountWithDict:responseObject];
          
          // 5.存储模型数据
          [HQAccountTool saveAccount:account];
          
          // 6.新特性\去首页
          [HQWeiboTool chooseRootController];
