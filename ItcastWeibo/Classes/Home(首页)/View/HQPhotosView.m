...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            HQPhotoView *photoView = [[HQPhotoView alloc] init];
...
- (void)photoTap:(UITapGestureRecognizer *)recognizer
{
    ...
    for (int i = 0; i<count; i++) {
        ...
        HQPhoto *HQphoto = self.photos[i];
        NSString *photoUrl = [HQphoto.thumbnail_pic stringByReplacingOccurrencesOfString:@"thumbnail" withString:@"bmiddle"];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        HQPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            CGFloat photoX = col * (HQPhotoW + HQPhotoMargin);
            CGFloat photoY = row * (HQPhotoH + HQPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, HQPhotoW, HQPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * HQPhotoH + (rows - 1) * HQPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * HQPhotoW + (cols - 1) * HQPhotoMargin;
