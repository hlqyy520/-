...
#define HQNewfeatureImageCount 3

@interface HQNewfeatureViewController () <UIScrollViewDelegate>
@property (nonatomic, weak) UIPageControl *pageControl;
@end

@implementation HQNewfeatureViewController
- (void)viewDidLoad
{
    [super viewDidLoad];
...
- (void)setupPageControl
{
    ...
    pageControl.numberOfPages = HQNewfeatureImageCount;
    ...
    // 2.设置圆点的颜色
    pageControl.currentPageIndicatorTintColor = HQColor(253, 98, 42);
    pageControl.pageIndicatorTintColor = HQColor(189, 189, 189);
}
...
- (void)setupScrollView
{
    ...
    for (int index = 0; index<HQNewfeatureImageCount; index++) {
        ...
        // 在最后一个图片上面添加按钮
        if (index == HQNewfeatureImageCount - 1) {
            [self setupLastImageView:imageView];
        }
    }
    
    // 3.设置滚动的内容尺寸
    scrollView.contentSize = CGSizeMake(imageW * HQNewfeatureImageCount, 0);
...
- (void)start
{
    ...
    // 切换窗口的根控制器
    self.view.window.rootViewController = [[HQTabBarViewController alloc] init];
}
